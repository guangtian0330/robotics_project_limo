...
def measurement_model(data, pose, occupied_indices, MAP):
    ...
    xy = utils.dist_to_xy(lidar_data[obstacle], data.lidar_angles[obstacle])
...
def odometry_model(prev, curr, odom_prev, odom_curr):
    ...
    delta_rot1 = np.arctan2(odom_curr.y - odom_prev.y, odom_curr.x - odom_prev.x) - odom_prev.theta
    delta_trans = np.linalg.norm([odom_curr.x - odom_prev.x, odom_curr.y - odom_prev.y])
    delta_rot2 = odom_curr.theta - odom_prev.theta - delta_rot1
